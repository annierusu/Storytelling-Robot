...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<gesture_record-request>)))
  ...
  (cl:format cl:nil "# an array of robot parts (e.g. ['left_arm', 'right_arm'])~%string[] parts~%~%# make parts idle~%bool idleParts~%~%# wait for desired seconds before starting record~%uint8 wait~%~%# max recording time in second~%uint8 timeout~%~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'gesture_record-request)))
  "Returns full string definition for message of type 'gesture_record-request"
  (cl:format cl:nil "# an array of robot parts (e.g. ['left_arm', 'right_arm'])~%string[] parts~%~%# make parts idle~%bool idleParts~%~%# wait for desired seconds before starting record~%uint8 wait~%~%# max recording time in second~%uint8 timeout~%~%~%~%"))
